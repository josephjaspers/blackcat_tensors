...
//template<int x> struct tensor_of { template<class t> using type = Tensor_Base<t>;  	 };
//
//
////shorthand
//template<int x, class a> using tensor_of_t = typename tensor_of<x>::template type<a>;
...
template<class T> struct is_tensor_impl<Tensor_Base<T>> { static constexpr bool conditional =true; };
