...
public:
    ...
    const auto scalar(BC::size_t i) const { return make_tensor(expression_templates::make_scalar(internal(), i)); }
          auto scalar(BC::size_t i)       { return make_tensor(expression_templates::make_scalar(internal(), i)); }

	const auto slice(BC::size_t i) const {
		return make_tensor(expression_templates::make_slice(internal(), i));
	}
	...
	const auto slice(BC::size_t from, BC::size_t to) const {
		return make_tensor(expression_templates::make_ranged_slice(internal(), from, to));
	}
    ...
    const auto diag(BC::size_t index = 0) const {
        ...
        return make_tensor(expression_templates::make_diagnol(internal(),index));
    }
    ...
    const auto row(BC::size_t i) const {
        ...
        return make_tensor(expression_templates::make_row(internal(), i));
    }
...
const auto reshape(const Tensor_Base<T>& tensor) {
    return [&](auto... integers) {
        return make_tensor(expression_templates::make_view(tensor.internal_base(), BC::make_array(integers...)));
    };
}
template<class T>
...
const auto chunk(const Tensor_Base<T>& tensor, integers... ints) {
    ...
    return [&, index_point](auto... shape_indicies) {
        return make_tensor(expression_templates::make_chunk(
