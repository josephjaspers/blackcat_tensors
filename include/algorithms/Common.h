...
struct ReferenceIterator:
    ...
    T& operator*() const { return *(parent::operator*()); }
    T& operator[](std::size_t index) const { return *(parent::operator[](index)); }
};
...
struct ReferenceList {
	...
	auto begin() { return ReferenceIterator<T>(container.begin()); }
	auto end() { return ReferenceIterator<T>(container.end()); }
