...
struct Forward_LayerCache {
	...
	cache_value_type m_cache;
	batched_cache_value_type m_batched_cache;
...
private:
	...
	cache_value_type& cache_impl(const Tensor_Base<Expression>& expression, BC::traits::Integer<Dimension>) {
		...
		//maybe add check
		if (m_cache.size() == expression.size()) {
			m_cache = expression;
		} else {
			m_cache = cache_value_type(expression); //calls move operator (resizes)
		}
		return m_cache;
	}
	template<class Expression>
	batched_cache_value_type& cache_impl(const Tensor_Base<Expression>& expression, BC::traits::Integer<Dimension+1>) {
		if (m_batched_cache.size() == expression.size()) {
			m_batched_cache = expression;
		} else {
			m_batched_cache = batched_cache_value_type(expression); //calls move operator (resizes)
		}
