...
class Stream<host_tag> {
	...
	{
		static thread_local std::shared_ptr<Contents> default_contents =
...
public:
	...
	auto get_allocator_rebound()
	{
		return typename allocator_type::
				template rebind<RebindType>::other(m_contents->m_workspace);
	}
	...
	void wait_event(Stream& stream)
	{
		if (!stream.is_default()) {
			BC_ASSERT(
					stream.m_contents->m_event.get(),
					"Attempting to wait on an event that was never recorded");
