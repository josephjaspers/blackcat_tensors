...
struct NeuralNetworkTest {
	...
	NeuralNetworkTest(layers... l) : network(l...) {}
	...
	void updateWeights(const NeuralNetworkTest& n)   { update<0, length>(n); }
	void fastCopy(const NeuralNetworkTest& nn) { fCopy<0, length>(nn); }
...
private:
	...
	template<int start, int end, class T> auto fp  (const vec_expr<T>& param, std::true_type) {
		return param;
	}
	template<int start, int end, class T> auto fpx  (const vec_expr<T>& param, std::true_type) {
		return param;
	}
	template<int start, int end, class T> auto bp  (const vec_expr<T>& param, std::true_type) {
		return param;
	}
	template<int start, int end, class T> auto bptt  (const vec_expr<T>& param, std::true_type) {
		return param;
	}
	...
	//for multithreading
	template<int start, int end> void update  (const NeuralNetworkTest& cpy_net) {
		if (start != end) {
	...
	}	template<int start, int end> void fCopy(const NeuralNetworkTest& nn) {
...
template<class... layers>
static NeuralNetworkTest<layers&...> generateNetworkTest(layers&... l) {
	return NeuralNetworkTest<layers&...>(l...);
}
template<class... layers>
static NeuralNetworkTest<layers&...> generateNetworkTestRef(layers&... l) {
	return NeuralNetworkTest<layers&...>(l...);
}
template<class... layers>
static NeuralNetworkTest<layers...> generateNetworkTestCopy(layers... l) {
	return NeuralNetworkTest<layers...>(l...);
}
}
