 ...
 */
namespace BC {
	template<int ... dimensions> struct Static_Inner_Shape;
	template<int ... dimensions> struct Static_Outer_Shape;
	...
	struct DEFAULT_LD_impl<list<front, set...>> {
		...
		using type = typename extract<typename DEFAULT_LD_impl<list<set...>>::type, Static_Outer_Shape<value>>::type;
	};

	template<template<int...> class list, int front>
	struct DEFAULT_LD_impl<list<front>> {
		static constexpr int value = front;
		using type = Static_Outer_Shape<front>;
	};
