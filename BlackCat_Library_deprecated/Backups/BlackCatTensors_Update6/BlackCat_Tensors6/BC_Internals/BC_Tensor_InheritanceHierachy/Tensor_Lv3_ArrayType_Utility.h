...
struct BC_Tensor_Utility_Array
		: public Tensor_Type<T, derived, math_lib, Shape<inner_shape, outer_shape>, BC_MTF::is_same<outer_shape, typename DEFAULT_LD<inner_shape>::type>::conditional>
{
	using this_type =     BC_Tensor_Utility_Array<T, derived, math_lib, inner_shape, outer_shape>;
	using parent_class =  Tensor_Type<T, derived, math_lib, Shape<inner_shape, outer_shape>, BC_MTF::is_same<outer_shape, typename DEFAULT_LD<inner_shape>::type>::conditional>;
	...
	void print() const {
		math_lib::print(this->array, this->getShape(), Shape<inner_shape, outer_shape>::RANK, 5);
	}
};
}
