...
namespace MTF {
		...
		template<class T> struct isRef 	   { static constexpr bool conditional = false; };
		template<class T> struct isRef<T&> { static constexpr bool conditional = false; };
...
namespace INT {
	...
	template<class T> struct remove_mods<T&&> { using type = typename remove_mods<T>::type; };
	template<class T> struct remove_mods<T*>  { using type = typename remove_mods<T>::type; };
	template<class T> struct remove_mods<const T&>  { using type = typename remove_mods<T>::type; };
	template<class T> struct remove_mods<const T&&> { using type = typename remove_mods<T>::type; };
	template<class T> struct remove_mods<const T*>  { using type = typename remove_mods<T>::type; };
