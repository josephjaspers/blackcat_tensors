...
//	const int* OuterShape() const 			{ return left().OuterShape(); }
	const auto addressOf(int offset) const { return binary_express_scalar_R(addressOf(left, offset), addressOf(right)); }
...
public:
		...
		void printLDDimensions()	const { left.printLDDimensions(); }
		auto accessor_packet(int index) const { return left.accessor_packet(index); }
//		const int* InnerShape() const 			{  std::cout << " inner shape - scalLR " << std::endl;  return left().InnerShape(); }
//		const int* OuterShape() const 			{ return left().OuterShape(); }
		const auto addressOf(int offset) const { return binary_express_scalar_LR(addressOf(left), addressOf(right)); }
