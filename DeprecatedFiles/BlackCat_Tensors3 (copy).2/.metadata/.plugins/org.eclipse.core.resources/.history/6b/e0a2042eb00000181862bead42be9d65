...
public:
	...
	const evaluation_type eval() const { return (this->asBase()); }
		  evaluation_type eval() 	   { return (this->asBase()); }
		  		...
		  		static constexpr bool trunc_params() { return dp_impl<b, t>::short_params; }
		  		static constexpr bool trunc_self() { return dp_impl<derived, TRAITS>::short_params; }
	...
	template<class pDeriv, class pTraits, class voider = typename std::enable_if<!dp_impl<pDeriv,pTraits>::short_params && !dp_impl<pDeriv,pTraits>::short_params>::type, int foo = 0>
	typename dp_impl<pDeriv, pTraits>::type operator *(const Tensor_Operations_impl<pDeriv, pTraits>& param) const {
