...
struct Array_Row : Array_Base<Array_Row<PARENT>, 1> {
	...
	__BCinline__ Array_Row(PARENT parent_, int index) :  parent(parent_), array_slice(const_cast<scalar_t*>(&(parent_.memptr()[index]))) {}
	...
	__BCinline__ const auto block_shape() const { return l_array<1>([&](int i) { return this->block_dimension(i); }); }
	...
	__BCinline__ int leading_dimension(int i) const { return i == 0 ? parent.leading_dimension(0) : 0; }
	__BCinline__ int block_dimension(int i) const { return i == 0 ? leading_dimension(0) : 1; }
...
auto make_row(internal_t internal, int index) {
	 return Array_Row<internal_t>(internal, index);
}
}
