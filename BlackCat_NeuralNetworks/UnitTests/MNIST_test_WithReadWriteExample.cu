...
int percept_MNIST() {
	...
	std::cout << "\n \n \n this is the trained network output ----------------------------------------------" << std::endl;
	...
	FeedForward F(1000,1000); //dimensions are irrevalant matter (they will be overwritten)
	auto network_to_read_from = generateNetwork(F,F);

	//read the stored network
	std::ifstream is("NN_ReadWriteTest.txt");
	network_to_read_from.read(is);
	is.close();

	std::cout << "\n \n \n this is the network that we read from a file ----------------------------------------------" << std::endl;

	//test the network again to ensure the read was successful
	for (int i = 0; i < 3; ++i) {

		std::cout << " output " << std::endl;
		outputs[i].print();
		std::cout << "prediction " << std::endl;
		network_to_read_from.forwardPropagation(inputs[i]).print();
		std::cout << "-----------------------------------------------------------------------------------------------------------" << std::endl;
	}
...
int main() {
	...
	BC::ReadWriteTest::percept_MNIST();
