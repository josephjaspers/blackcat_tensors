...
private:
	...
	template<int start, int end, class T> vec fp  (vec_expr<T> param) {
		if (start != end)
			return fp  <min<start + 1, end>::value, end, T>(network.get<min<start, end -1>::value>().forwardPropagation(param));
		else
			return param;
	}
	template<int start, int end, class T> vec fpx  (vec_expr<T> param) {
		if (start != end)
			return fpx  <min<start + 1, end>::value, end, T>(network.get<min<start, end -1>::value>().forwardPropagation_express(param));
		else
			return param;
	}
	template<int start, int end, class T> vec bp  (vec_expr<T> param) {
		if (start > end)
			return bp  <max<start - 1, end>::value, end, T>(network.get<max<start - 1, end>::value>().backPropagation(param));
		else
			return param;
	}
	template<int start, int end, class T> vec bptt  (vec_expr<T> param) {
		if (start > end)
			return bptt  <max<start - 1, end>::value, end, T>(network.get<max<start - 1, end>::value>().backPropagationThroughTime(param));
		else
			return param;
	}
